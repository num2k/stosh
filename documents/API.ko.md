...
stosh는 localStorage나 sessionStorage를 사용할 수 없는 환경(예: 프라이빗 모드, 저장소 용량 초과, 브라우저 제한, 비브라우저 환경 등)에서 자동으로 메모리 스토리지로 전환되어 동작합니다. 이 경우, 데이터는 브라우저를 새로고침하거나 탭을 닫으면 사라집니다.
