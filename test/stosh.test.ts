...
describe('Stosh 통합 테스트', () => {
  ...
  it('미들웨어 캐싱 동작 검사', () => {
    const st = stosh({ type: 'idb' });
    const mw1 = jest.fn((ctx, next) => next());
    const mw2 = jest.fn((ctx, next) => next());
    ...
    // 동일한 미들웨어 리스트를 재사용하는지 확인 (캐싱 효과)
    st.set('key2', 'value2');
    expect(mw1).toHaveBeenCalledTimes(2);
    expect(mw2).toHaveBeenCalledTimes(2);
  });

  it('미들웨어가 없을때 바로가기 경로', () => {
    const st = stosh({ type: 'idb' });
    const lastFn = jest.fn((ctx) => {
      (ctx as any).result = 'direct result';
    });

    // 미들웨어 없이 바로 last 호출 확인
    const ctx = { key: 'key' };
    st['runMiddlewareSync']('get', ctx, lastFn);
    expect(lastFn).toHaveBeenCalledTimes(1);
    expect((ctx as any).result).toBe('direct result');
  });
